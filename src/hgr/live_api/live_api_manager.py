...
def build_system_instructions() -> str:
    ...
    return (
        ...
        "truncate a real inbox). After the call: report the EXACT count it "
        "returned (`result.count` for gmail_list) and summarize ONLY the "
        "messages in `result.messages` — read each entry's from_name, "
        "subject, snippet/body_text VERBATIM. NEVER invent senders, "
        "subjects, or details that aren't in that array — if a field is "
        "blank, say it's blank; if the array is empty, say there are no "
        "unread emails. If the user has more unread than max=50 fetched, "
        "say 'these are the first 50 of N' (use the count). Only if "
        "find_capability returns NOTHING (no email connector wired) fall "
        "back to open_app('outlook') + read_screen. Do NOT default to "
        "opening Outlook + read_screen when a connector is available — "
        "that path is slow, brittle, and the user explicitly does not "
        "want it. Do NOT compose a new email when asked to READ one. To "
        "read ONE email's full body via the connector pass include_body="
        "true on the list call (or gmail_read with the id) instead of "
        "clicking into the message in a window.\n"

...
class LLMPlanner:
    ...
    def _build_messages(self, goal: str,
        ...
        system = (
            ...
            "MEMORY-RESOLVED CONTACTS: if the context block lists a "
            "'person <name> = <email>' fact that matches the recipient in "
            "the request, USE THAT EMAIL DIRECTLY — skip contacts_search "
            "entirely. Example: context says 'person dani = "
            "dani@mangollc.org' and user says 'send Dani a quick hi'; the "
            ...
            'subject + a one-line summary of the body. Do not truncate the '
            'list; include all emails from the input.",'
            '"inputs":"Weather: {step:1.summary}\\nUnread emails '
            '({step:2.count}):\\n{step:2.messages}",'
            '"max_tokens":2500},"depends_on":[1,2]},'
    ...
    def _tool_catalog(self) -> str:
        ...
        lines.extend([
            ...
            "- weather_get [iris]  Free wttr.in / Open-Meteo lookup, "
            "single API call. Use for ANY 'weather / forecast / "
            "temperature / is it raining / will it rain / forecast for "
            "next N days / this week / tomorrow' request — NEVER fall "
            "back to web_search/web_navigate for weather. For temporal "
            "phrasings like 'forecast for the next three days' or "
            "'weather tomorrow', pass location=\"\" (empty) — temporal "
            "phrases are NOT place names. ALWAYS pass `days` matching "
            "what the user asked for: 'four days' → days=4, 'next "
            "week' → days=7, 'weekend' → days=2, 'tomorrow' → days=2, "
            "no horizon mentioned → days=3 (default). Capped at 10. "
            "Args: location (optional — empty = auto IP geolocate), "
            "days (1-10, default 3). Returns: {status, location, "
            "description, temperature, feels_like, humidity_pct, wind, "
            "summary, forecast[]}. `forecast` is a list of "
            "{day, min, max, description} entries. WRITE THE REPLY "
            "YOURSELF in your normal Jarvis voice using this data — "
            "natural, conversational, covers however many days the "
            "user asked for, no robotic 'today: X. tomorrow: Y.' "
            "templates, no dates spoken aloud (use 'tomorrow' / "
            "weekday names), no URLs read out. Only fall back to "
            "{step:N.summary} verbatim if you can't compose a fresh "
            "reply for some reason. When chaining weather INTO an "
            "email/message body, use {step:N.summary} (NOT temp_f / "
